...
const PhrasesExpressions = () => {
  ...
  useEffect(() => {
    ...
    if (showFavorites) {
      filtered = phrasesData.phrases.filter((phrase) =>
        favoriteIds.includes(phrase.id)
      );
    }
    // Sinon, filtrer par catégorie et sous-catégorie
    else if (selectedCategory) {
      filtered = phrasesData.phrases.filter(
        (phrase) => phrase.categoryId === selectedCategory
      );

      if (selectedSubcategory) {
        filtered = filtered.filter(
          (phrase) => phrase.subcategoryId === selectedSubcategory
        );
      }
    }
  ...
  const checkAnswer = () => {
    ...
    if (practiceMode === "translation") {
      ...
      isCorrect =
        userAnswer.toLowerCase().trim() ===
        missingWords.join(" ").toLowerCase().trim();
    ...
    <>
      ...
      {!showFavorites && phrasesData && phrasesData.categories && (
        <>
          ...
          {selectedCategory &&
            phrasesData.categories.find((c) => c.id === selectedCategory)
              .subcategories &&
            phrasesData.categories.find((c) => c.id === selectedCategory)
      ...
      <Text style={styles.resultCount}>
        ...
        {filteredPhrases.length === 1 ? "phrase" : "phrases"} found
      </Text>
    ...
    if (filteredPhrases.length === 0) {
      return (
        <View style={styles.emptyPracticeContainer}>
          <Text style={styles.emptyPracticeText}>
            No phrases available for practice.
          </Text>
          <TouchableOpacity
      ...
      >
          ...
          <View style={styles.progressBar}>
            <View
              style={[
                ...
                {
                  width: `${
                    (currentPracticeIndex / filteredPhrases.length) * 100
                  }%`,
        ...
        <View style={styles.practiceCard}>
          ...
          {practiceMode === "fillin" && (
            <>
              ...
              <Text style={styles.practicePhrase}>
                {currentPhrase.fillInSentence ||
                  currentPhrase.english.replace(/_+/g, "_____")}
              </Text>
          ...
          {practiceMode === "arrange" && (
            <>
              <Text style={styles.practicePrompt}>
                Arrange the words to form the correct phrase:
              </Text>
              ...
              <View style={styles.arrangedSentenceContainer}>
                {selectedOptions.length > 0 ? (
                  <View style={styles.arrangedWords}>
                    {selectedOptions.map((word, index) => (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          !showAnswer && handleRemoveWord(word, index)
                        }
                        disabled={showAnswer}
                      >
                        <Text
                  ...
                  <Text style={styles.arrangedPlaceholder}>
                    Tap words below to form the phrase
                  </Text>
                )}
              </View>
              ...
              {!showAnswer && (
                <TouchableOpacity
                  style={[
                    styles.resetArrangementButton,
                    { borderColor: levelColor },
                  ]}
                  onPress={resetArrangement}
                >
                  <Text
                    style={[styles.resetArrangementText, { color: levelColor }]}
                  >
                    Reset
                  </Text>
                </TouchableOpacity>
              ...
              <Text style={styles.answerFeedbackTitle}>
                ...
                (practiceMode === "translation"
                  ...
                  ? (currentPhrase.missingWords || [])
                      ...
                      .toLowerCase()
                      .trim()
                  : selectedOptions.join(" ").toLowerCase().trim() ===
                    currentPhrase.english.toLowerCase().trim()
                  ? currentPhrase.english.toLowerCase().trim()
                  : "")
                  ? "Correct!"
        ...
        <View style={styles.practiceActions}>
          ...
          >
            <Text
              style={[styles.exitPracticeButtonText, { color: levelColor }]}
            >
              Exit Practice
            </Text>
          </TouchableOpacity>
